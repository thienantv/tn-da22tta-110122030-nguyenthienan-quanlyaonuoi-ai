...
const sopService = {
   ...
   * Tích hợp: Tịnh tiến ngày bắt đầu & Chống trùng lặp thông minh
   */
  generateSOPFromTemplate: async (seasonId, pondId, startDate, expectedHarvestDate, technicianId, templateConfig) => {
    ...
    // 1. CHUẨN HÓA NGÀY GIỜ (Khử sai số do múi giờ)
    const startD = new Date(startDate);
    startD.setHours(0, 0, 0, 0);

    const endD = expectedHarvestDate ? new Date(expectedHarvestDate) : new Date(startD.getTime() + (120 * 86400000)); 
    endD.setHours(0, 0, 0, 0);

    const totalDays = Math.floor((endD.getTime() - startD.getTime()) / 86400000);
    if (totalDays <= 0 || totalDays > 200) return; 

    const today = new Date();
    today.setHours(0, 0, 0, 0);

    // 2. TÍNH TOÁN NGÀY BẮT ĐẦU VÒNG LẶP (Chỉ sinh task từ ngày mai nếu vụ đang chạy)
    let startingDayIndex = 1; 
    let generateFromDate = new Date(startD);

    if (today >= startD) {
        const tomorrow = new Date(today);
        tomorrow.setDate(tomorrow.getDate() + 1);
        
        const diffTime = tomorrow.getTime() - startD.getTime();
        startingDayIndex = Math.floor(diffTime / 86400000) + 1; 
        generateFromDate = new Date(tomorrow);
    }

    // Nếu mốc bắt đầu tạo đã vượt quá ngày thu hoạch thì dừng
    if (startingDayIndex > totalDays) return;

    // =====================================================================
    // 3. CƠ CHẾ CHỐNG TRÙNG LẶP (SMART CONFLICT RESOLUTION)
    // =====================================================================
    // Quét toàn bộ công việc Kỹ sư đã tự tạo thủ công từ 'generateFromDate' trở đi
    const existingTasksRes = await pool.query(`
        SELECT type_id, start_date::date AS task_date 
        FROM tasks 
        WHERE season_id = $1 AND start_date >= $2 AND status != 'CANCELLED'
    `, [seasonId, generateFromDate]);

    // Đưa vào Set (Từ điển) dạng "YYYY-MM-DD_TypeID" để tra cứu tốc độ cao
    const existingTaskSet = new Set();
    existingTasksRes.rows.forEach(row => {
        const d = new Date(row.task_date);
        const dateStr = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
        existingTaskSet.add(`${dateStr}_${row.type_id}`);
    });

    // =====================================================================
    // 4. VÒNG LẶP SINH TASK (BỎ QUA NẾU ĐÃ TỒN TẠI)
    // =====================================================================
    const tasksToInsert = [];
    
    for (let day = startingDayIndex; day <= totalDays; day++) {
      const currentDay = new Date(generateFromDate);
      currentDay.setDate(currentDay.getDate() + (day - startingDayIndex));

      // Lấy chuỗi ngày an toàn, không bị ảnh hưởng bởi múi giờ (Timezone safe)
      ...
      if (templateConfig['5'] && !existingTaskSet.has(`${dateString}_5`)) {
      ...
      // 2. Cho ăn (Type: 2)
      if (templateConfig['2'] && !existingTaskSet.has(`${dateString}_2`)) {
      ...
      if (templateConfig['3'] && !existingTaskSet.has(`${dateString}_3`)) {
      ...
      if (templateConfig['4'] && day >= 30 && !existingTaskSet.has(`${dateString}_4`)) {
    ...
    // 5. Thu hoạch (Type: 6)
    if (templateConfig['6']) {
        const harvestYear = endD.getFullYear();
        const harvestMonth = String(endD.getMonth() + 1).padStart(2, '0');
        const harvestDay = String(endD.getDate()).padStart(2, '0');
        const harvestDateString = `${harvestYear}-${harvestMonth}-${harvestDay}`;
        
        if (!existingTaskSet.has(`${harvestDateString}_6`)) {
    ...
    if (tasksToInsert.length === 0) {
        console.log(`✅ [SOP ENGINE] Mọi công việc đã được Kỹ sư tự lên lịch đầy đủ. Không cần sinh thêm SOP.`);
        return;
    }

    // BƯỚC 5: TRANSACTION LƯU DỮ LIỆU VÀO DATABASE
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      
      // Lấy thông tin giá tiền vật tư từ Database để dự trù
      const productIds = [];
      ...
      // LƯU CÔNG NHÂN VÀ VẬT TƯ (Dựa theo Mẫu Config)
      const workerParams = [];
      const workerValues = [];
      let wIdx = 1;
      ...
      console.log(`✅ [SOP ENGINE] Đã bổ sung thành công ${tasksToInsert.length} Task SOP cho Mùa vụ ${seasonId}`);
    } catch (error) {
